...
void prim(int start_point)
{
    ...
    {
        min = INT_MAX;
        for (j = 1; j <= num_V; j ++)               /* 找出不在集合中的距离集合最近的点 */
        ...
        if (min > ans) ans = min; //poj
        for (j = first[point]; j > 0; j = next[j])  /* 更新此点与其它点的距离为到集合的距离 */
            if (mark[end[j]] == FALSE && value[j] < distance[end[j]])
                distance[end[j]] = value[j];
    }
    printf("%d\n", ans); //poj
}
